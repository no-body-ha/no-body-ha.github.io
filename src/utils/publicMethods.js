...
export function fileToImg(name) {
  ...
  ctx.roundRect(0, 0, textWidth + 20, 44, 5)
...
// 计算占位图的宽高
export function calcImgSize(width, height, maxWidth = 600, minWidth = 150, maxHeight = 600, minHeight = 150) {
  if (width > maxWidth) {
...
export function getFileByAlt(content, fileList) {
  ...
  return content.replace(reg, (res, $1) => {
    ...
    const fileTag = `<img src="${file.fileSrc}" alt="[&*${$1}]" style"${file.fileType === 'attachment' ? 'height:auto;' : 'max-height:600px; max-width:600px;'} vertical-align:bottom;  padding:8px; display:block;">`
    return fileTag
  })
}
