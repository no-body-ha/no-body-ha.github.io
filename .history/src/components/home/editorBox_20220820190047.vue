...
import "@/utils/codeMirrorSetting.js";
import "codemirror/mode/javascript/javascript"; // 这里引入的模式的js，根据设置的mode引入
...
export default {
  ...
  mounted() {
      ...
      if (
        ...
        for (let value of e.path) {
          ...
          if (value == this.$refs.titleDom || value == this.$refs.textDom) {
            ...
            if (value.style.textAlign == "left") {
              Middleware.$emit("AlignLeft", true);
            } else if (value.style.textAlign == "center") {
              Middleware.$emit("Center", true);
            } else if (value.style.textAlign == "right") {
              Middleware.$emit("AlignRight", true);
            }
            Middleware.$emit(
              "fontColor",
              value.style.color || value.style.fontColor || "#000000"
    ...
    // 监听初始化 代码块
    Middleware.$on("initCodeBlock", (data) => {
      this.$nextTick(() => {
        this.initCodeBlock(data);
      });
    });
  },
  methods: {
    ...mapMutations(["setPageBackground", "setPageScale"]),
    // 初始化 代码块 为 codeMirror
    initCodeBlock(codeBlockDom) {
      // console.log(codeBlockDom);
      // 初始化编辑器实例，传入需要被实例化的文本域对象和默认配置
      this.coder = CodeMirror.fromTextArea(codeBlockDom, this.cmOption);
      // console.log(this.coder);
      // 编辑器赋值
      // this.coder.setValue(codeBlockDom.value);
      // 支持双向绑定
      this.coder.on("change", (coder) => {
        let code = coder.getValue();
        codeBlockDom.innerHTML = code;
      });
    },
    // 防止输入框失焦的回调
    blurTitle() {
